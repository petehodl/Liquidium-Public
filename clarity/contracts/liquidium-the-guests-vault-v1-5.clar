...
;;;; liquidium-the-guests-vault-v1-5
;;;; nft: the-guests
...
(define-private (close-auction (auctionId uint) (count uint))
    (let
        ...
        (asserts! (is-ok (as-contract (contract-call? .the-guests transfer auctionAssetId tx-sender auctionLastBidderAccount)))
            count
        )
        ...
        (let
            ...
            (if (is-eq newLoanDebtBalance u0)
                (begin
                    ...
                    (var-set activeLoanIds (filter not-tempUint (var-get activeLoanIds)))
                    (map-set Borrower borrowerAccount (filter not-tempUint borrowerActiveLoanIds))
                    (unwrap! (as-contract (contract-call? .the-guests transfer loanAssetId tx-sender borrowerAccount)) ERR_ASSET_TRANSFER_FAILED)
                    (ok true)
                )
                (ok true)
            )
...
(define-public (new-loan (assetId uint) (amount uint) (termLength uint))
    (begin
        (asserts! (is-eq (ok (some tx-sender)) (contract-call? .the-guests get-owner assetId))
            ERR_ASSET_NOT_OWNED
        )
        ...
        (let
            ...
            (asserts! (is-ok (contract-call? .the-guests transfer assetId borrowerAccount (as-contract tx-sender)))
...
(define-public (withdrawl-the-guests (id uint))
    (begin
        ...
        (unwrap! (as-contract (contract-call? .the-guests transfer id tx-sender LIQUIDIUM_ACCOUNT)) ERR_ASSET_TRANSFER_FAILED)
        (ok true)
    )
)
